...
public class TableCodecV2 {
    ...
    protected static <K> Object[] decodeObjects(byte[] value, K handle, TiTableInfo tableInfo) {
        ...
        for (TiColumnInfo col : tableInfo.getColumns()) {
            if (col.isPrimaryKey() && (tableInfo.isPkHandle() || tableInfo.isCommonHandle())) {
                decodedDataMap.put(col.getId(), handle);
                continue;
            }
            org.tikv.common.codec.RowV2.ColIDSearchResult searchResult =
                    rowV2.findColID(col.getId());
    ...
    protected static <K> Row decodeRow(byte[] value, K handle, TiTableInfo tableInfo) {
