...
public class CatalogTransaction {
    protected static final Logger LOG = LoggerFactory.getLogger(CatalogTransaction.class);
    ...
    public static <T> T parseFromJson(ByteString json, Class<T> cls) {
        ...
        LOG.debug(String.format("Parse Json %s : %s", cls.getSimpleName(), json.toStringUtf8()));
        ...
        for (Pair<ByteString, ByteString> pair : fields) {
            if (KeyUtils.hasPrefix(pair.first, ByteString.copyFromUtf8(MetaCodec.KEY_TABLE))) {
                try {
                    ...
                    LOG.warn("fail to parse table from json!", e);
                }
            }
        }
        return builder.build();
