...
// copied from org.apache.flink.table.data.binary.MurmurHashUtils;
...
public final class MurmurHashUtils {
    ...
    /**
     * Hash byte buffer, length must be aligned to 4 bytes.
     *
     ...
     * @return hash code
     */
    public static int hashUnsafeBytesByWords(ByteBuffer buf, long offset, int lengthInBytes) {
        return hashUnsafeBytesByWords(buf, offset, lengthInBytes, DEFAULT_SEED);
    }
    ...
    private static int hashUnsafeBytesByWords(
            ByteBuffer buf, long offset, int lengthInBytes, int seed) {
        int h1 = hashUnsafeBytesByInt(buf, offset, lengthInBytes, seed);
        return fmix(h1, lengthInBytes);
    }
    ...
    private static int hashUnsafeBytesByInt(
            ByteBuffer buf, long offset, int lengthInBytes, int seed) {
        assert (lengthInBytes % 4 == 0);
        int h1 = seed;

        for (int i = 0; i < lengthInBytes; i += 4) {
            int halfWord = buf.get((int) (offset + i));
            int k1 = mixK1(halfWord);
            h1 = mixH1(h1, k1);
        }
        return h1;
